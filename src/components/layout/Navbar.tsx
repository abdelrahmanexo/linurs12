...
import { Menu, X, LayoutDashboard, Home, GraduationCap, LogIn, LogOut, Lock, Mail, Eye, EyeOff, Loader2, ShieldAlert } from 'lucide-react'
import { signInWithEmailAndPassword, signOut, onAuthStateChanged } from 'firebase/auth'
import { auth } from '@/lib/firebase'

const ADMIN_EMAIL = 'nursingryadaeg@gmail.com'
...
export default function Navbar() {
    ...
    useEffect(() => { setIsOpen(false); setShowLoginDropdown(false) }, [pathname])

    useEffect(() => {
        if (!blockedUntil) return
        const iv = setInterval(() => {
            const rem = blockedUntil - Date.now()
            if (rem <= 0) { setBlockedUntil(null); setAttempts(0); setCountdown(''); clearInterval(iv) }
            else {
    ...
    const handleLogin = useCallback(async (e: React.FormEvent) => {
        e.preventDefault()
        setError('')
        if (isBlocked) { setError(`محظور مؤقتاً، حاول بعد ${countdown}`); return }

        if (email.trim().toLowerCase() !== ADMIN_EMAIL) {
            const n = attempts + 1
            setAttempts(n)
            if (n >= MAX_ATTEMPTS) { setBlockedUntil(Date.now() + BLOCK_DURATION); setError('تم حظرك مؤقتاً لمدة ١٥ دقيقة') }
            else setError(`غير مصرح لك بالدخول (${MAX_ATTEMPTS - n} محاولات متبقية)`)
            return
        }
        ...
        try {
            ...
            setAttempts(n)
            if (n >= MAX_ATTEMPTS) { setBlockedUntil(Date.now() + BLOCK_DURATION); setError('تم حظرك مؤقتاً لمدة ١٥ دقيقة') }
            else setError(`كلمة المرور غلط (${MAX_ATTEMPTS - n} محاولات متبقية)`)
        } finally { setLoading(false) }
    }, [email, password, router, attempts, blockedUntil, countdown, isBlocked])
    ...
    return (
        <header dir="rtl" className={`fixed top-0 inset-x-0 z-[100] transition-all duration-700 ${scrolled ? 'py-3 md:py-4 px-4' : 'py-6 md:py-8 px-6'}`}>
            <nav className={`max-w-7xl mx-auto transition-all duration-700 border ${scrolled
                    ...
                    <div className="hidden md:flex items-center gap-1 bg-slate-100/30 backdrop-blur-sm p-1.5 rounded-2xl border border-white/20">
                        {navLinks.map((link) => {
                            ...
                            return (
                                <Link key={link.href} href={link.href} prefetch={true}
                                    ...
                                    {isActive && <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-600 shadow-lg shadow-blue-200 animate-in fade-in zoom-in duration-500 -z-10"></div>}
                                    <span className={`transition-transform duration-500 ${isActive ? 'scale-110' : 'group-hover:rotate-12'}`}>{link.icon}</span>
                                    {link.name}
                                </Link>
                            )
                        })}
                        {user && (
                            <Link href="/admin" prefetch={true}
                                ...
                                {pathname === '/admin' && <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-600 shadow-lg shadow-blue-200 -z-10"></div>}
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative hidden md:block" ref={dropdownRef}>
                            {user ? (
                                ...
                                <button onClick={() => { setShowLoginDropdown(!showLoginDropdown); setError('') }}
                                    ...
                                    <form onSubmit={handleLogin} className="space-y-4">
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                disabled={isBlocked}
                                                className="w-full pr-11 pl-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 text-sm placeholder:text-slate-300 outline-none focus:ring-4 ring-blue-500/10 focus:border-blue-400 transition-all text-right disabled:opacity-50"
                                                required
                                            />
                                        </div>
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                disabled={isBlocked}
                                                className="w-full pr-11 pl-11 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 text-sm placeholder:text-slate-300 outline-none focus:ring-4 ring-blue-500/10 focus:border-blue-400 transition-all text-right disabled:opacity-50"
                                                required
                                            />
                                        ...
                                        {error && (
                                            <div className="flex items-center gap-2 text-red-500 text-xs font-black bg-red-50 py-2 px-4 rounded-xl">
                                                <ShieldAlert size={14} />{error}
                                            </div>
                                        )}
                                        {isBlocked && (
                                            <div className="text-center text-orange-600 text-xs font-black bg-orange-50 py-2 px-4 rounded-xl">
                                                ⏳ انتظر {countdown}
                                            </div>
                                        )}
                ...
                <div className={`md:hidden absolute top-full inset-x-4 mt-4 transition-all duration-700 ${isOpen ? 'opacity-100 translate-y-0 visible scale-100' : 'opacity-0 -translate-y-10 invisible scale-95'}`}>
                    <div className="bg-white/95 backdrop-blur-3xl rounded-[2.5rem] p-8 shadow-[0_30px_60px_rgba(0,0,0,0.1)] border border-white flex flex-col gap-3">
                        ...
                        {navLinks.map((link) => (
                            <Link key={link.href} href={link.href} prefetch={true} onClick={() => setIsOpen(false)}
                        ...
                        {user && (
                            <Link href="/admin" prefetch={true} onClick={() => setIsOpen(false)}
                        ...
                        <div className="mt-4 pt-6 border-t border-slate-100 space-y-3">
                            {user ? (
                                ...
                                <form onSubmit={handleLogin} className="space-y-4">
                                    ...
                                    <button type="submit" disabled={loading || isBlocked}
                                        className="w-full py-5 bg-slate-900 hover:bg-blue-600 text-white rounded-[2rem] font-black shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-transform disabled:opacity-50">
                                        {loading
                                            ? <><Loader2 size={20} className="animate-spin" /> جاري الدخول...</>
                                            : <><LogIn size={20} /> دخول الأدمن</>}
                                    </button>
                                </form>
                            )}
                        </div>
